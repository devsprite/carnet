...
<div class="row box">
    ...
    {if !empty($carnet['activite_physique'])}
        <div class="box">
            ...
            {if $carnet['activite_physique'] == 'Oui_vous_avez_fait_l_effort'}<p>Oui, vous avez fait l'effort</p>{/if}
            {if $carnet['activite_physique'] == 'Pas_vraiment_mais_c_est_en_projet'}<p>Pas vraiment, mais c'est en projet</p>{/if}
            {if $carnet['activite_physique'] == 'Non_vous_etes_définitivement_faché'}<p>Non, vous êtes définitivement faché(e) avec le sport</p>{/if}
            {if $carnet['activite_physique'] == 'Non_vous_ne_pouvez_pas_pratiquer_d_activité_sportive'}<p>Non, vous ne pouvez pas pratiquer d'activité sportive (raisons de santé)</p>{/if}
            {if !empty($carnet['activite_physique_heure'])}<p>Vous avez pratiqué une activité physique durant {$carnet['activite_physique_heure']} heure(s)</p>{/if}
        </div>
    {/if}
    ...
    <div class="box">
        ...
        {if $carnet['alimentation_mange_entre_repas']}
            ...
            {if $carnet['alimentation_mange_entre_repas'] == 'Oui_collation'}<p>Oui, vous avez eu faim et pris des collations (en milieu de matinée ou milieu d'après-midi)</p>{/if}
            {if $carnet['alimentation_mange_entre_repas'] == 'Oui_grignote'}<p>Oui, vous avez grignoté par-ci par-là (hors collations)</p>{/if}
        {/if}
        {if $carnet['alimentation_repas_particulier']}
            <h4><strong>Avez-vous pris un ou plusieurs repas dans des situations particulières ? (Restaurant, repas de fête, invitation...)</strong></h4>
        ...
        {if $carnet['alimentation_frustration']}
            <h4><strong>Au niveau alimentaire cette semaine, quelles ont été vos sources de frustration ?</strong></h4>
    ...
    <div class="box">
        ...
        {if $carnet['programme_resolution']}
            <h4><strong>Notez ici une résolution que vous décidez de mettre en oeuvre dans la semaine qui vient : </strong></h4>
